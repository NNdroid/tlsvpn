...
func TestFrameScanner(t *testing.T) {
	...
	// 產生並寫入兩筆 Frame (不加密)
	buf1, _ := appendPaddedFrame(getFrame()[:0], VPNFrame{Seq: 101, Data: payload1}, nil)
	tcpBuffer.Write(buf1)

	buf2, _ := appendPaddedFrame(getFrame()[:0], VPNFrame{Seq: 102, Data: payload2}, nil)
...
func BenchmarkProtocolThroughput(b *testing.B) {
	...
	frameBuf := getFrame()[:0]
	frameBuf, _ = appendPaddedFrame(frameBuf, VPNFrame{Seq: 1, Data: payload}, rx)
