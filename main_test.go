...
func TestVSwitchStaticSessionMACFastPath(t *testing.T) {
	...
	vs.validateMAC = func(string, macKey) bool {
		validatorCalls++
		return false // static session path must never consult this callback
	}

	// 正常 session 帧只做 6B MAC 比较；目的 MAC 指向自己，不产生转发副作用。
	...
	// static session MAC 不参与 30 分钟动态老化。
	...
	// 冒充仍然必须被丢弃，而且同样不需要 activeClients/validateMAC 锁。
...
func BenchmarkVSwitchSourcePath(b *testing.B) {
	...
	b.Run("dynamic", func(b *testing.B) {
		...
		vs.ProcessFrame("A", frame) // warm learning table
		b.ReportAllocs()
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
